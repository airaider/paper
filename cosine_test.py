...
with open('movie_titles.txt', 'r') as f:
    for line in f:
        movie_titles2.append(line[:-1])
with open('rate.txt', 'r') as f:
    for line in f:
        rate_list.append(line[:-1])


count = 0
for i in movie_titles2:
    rate_dict[i] = rate_list[count]
    count+=1
...
def similar_movie(movie_title):
    ...
    ## making cosine vector (list) to string & marking tags
    cosine_vector_rounded = []
    indexes = []
    
    for i in range(0,len(feature_titles)):
        if (cosine_vector[i] > 0.01) :
            indexes.append(i)
            cosine_vector_rounded.append(format(cosine_vector[i],'.2f'))

    cosine_vector_str = ' '.join(cosine_vector_rounded) 
    #
    
    #tags
    tags = []
    for i in indexes:
        tags.append(feature_titles[i])
    tags_str = ' '.join(tags)
    #
    ...
    for i in range(len(movie_titles)):
        ...
        vector_dict[movie_titles[i]]=vector_list[i]
    ...
    for i in range(1,6):            
        ...
        cosine_vector_rounded = []
        indexes = []
        cosine_vector = vector_dict[rsorted_dict[i][0]]
        
        for l in range(0,len(feature_titles)):
            if (cosine_vector[l] > 0.01) :
                indexes.append(l)
                cosine_vector_rounded.append(format(cosine_vector[l],'.2f'))

        this_vec_str = '  '.join(cosine_vector_rounded)
        
        #tags
        tags = []
        for k in indexes:
            tags.append(feature_titles[k])
        this_tags_str = ' '.join(tags)
        #

    
        str_result += str(rsorted_dict[i][0])+"///"+ str(rsorted_dict[i][1]*100)[:5] + "///"+str(rate_dict.get(rsorted_dict[i][0])) + "///"

        file_path = './cine21/' + rsorted_dict[i][0] + '.txt' 
        with open(file_path, 'r', encoding='UTF8') as f:
            a = f.read()
            splited_list = a.split(".")
            stop = True
            sentences = ""
            sentences += splited_list[0] + "." + splited_list[1]+ "."  + splited_list[2] + '///'

        str_result += sentences + this_tags_str + "///" + this_vec_str + "///"



    #대조군 영화의 tag, vector
    str_result += tags_str + "///" + cosine_vector_str                
